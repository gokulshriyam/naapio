import silkImg from "@/assets/fabrics/silk.jpg";
import cottonImg from "@/assets/fabrics/cotton.jpg";
import linenImg from "@/assets/fabrics/linen.jpg";
import chiffonImg from "@/assets/fabrics/chiffon.jpg";
import georgetteImg from "@/assets/fabrics/georgette.jpg";
import velvetImg from "@/assets/fabrics/velvet.jpg";
import satinImg from "@/assets/fabrics/satin.jpg";
import crepeImg from "@/assets/fabrics/crepe.jpg";
import netImg from "@/assets/fabrics/net.jpg";
import denimImg from "@/assets/fabrics/denim.jpg";
import woolImg from "@/assets/fabrics/wool.jpg";
import organzaImg from "@/assets/fabrics/organza.jpg";
import brocadeImg from "@/assets/fabrics/brocade.jpg";
import vendorsChoiceImg from "@/assets/fabrics/vendors-choice.jpg";

export const fabricOptionsWithImages = [
  { name: "Silk", image: silkImg },
  { name: "Cotton", image: cottonImg },
  { name: "Linen", image: linenImg },
  { name: "Chiffon", image: chiffonImg },
  { name: "Georgette", image: georgetteImg },
  { name: "Velvet", image: velvetImg },
  { name: "Satin", image: satinImg },
  { name: "Crepe", image: crepeImg },
  { name: "Net / Tulle", image: netImg },
  { name: "Denim", image: denimImg },
  { name: "Wool", image: woolImg },
  { name: "Organza", image: organzaImg },
  { name: "Brocade", image: brocadeImg },
  { name: "Vendor's Choice", image: vendorsChoiceImg },
];
